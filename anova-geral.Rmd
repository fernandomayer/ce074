...
require(ExpDes)
fat2.crd(da.fat$adubo, da.fat$torta, da.fat$producao, sigF = 0.05)
...
##----------------------------------------------------------------------
## ANOVA usando a função aov() com Error()
m0 <- aov(var.resp ~ fatorA + fatorB + fatorA:fatorB +
              Error(parcela))
## Tabela ANOVA
summary(m0)
##----------------------------------------------------------------------
## Existe interação?
interaction.plot(fatorA, fatorB, var.resp)
lattice::wireframe(var.resp ~ fatorA*fatorB)
##----------------------------------------------------------------------
## Tabela de médias
model.tables(m0, type = "means")
## é o mesmo que:
mean(var.resp)
tapply(var.resp, fatorA, mean)
tapply(var.resp, fatorB, mean)
tapply(var.resp, fatorA:fatorB, mean)
##----------------------------------------------------------------------
## EFEITOS estimados
model.tables(m0, type = "effects")
## O que são os efeitos?
## Fator A
tapply(var.resp, fatorA, mean)
diff(tapply(var.resp, fatorA, mean))
diff(tapply(var.resp, fatorA, mean))/2
## fator B
tapply(var.resp, fatorB, mean)
diff(tapply(var.resp, fatorB, mean))
diff(tapply(var.resp, fatorB, mean))/2
## Interação
(int.med <- tapply(var.resp, fatorA:fatorB, mean))
((int.med["A1:T1"] + int.med["A0:T0"])/2 -
 (int.med["A1:T0"] + int.med["A0:T1"])/2)
((int.med["A1:T1"] + int.med["A0:T0"])/2 -
 (int.med["A1:T0"] + int.med["A0:T1"])/2)/2
## Os "efeitos" são então diferenças entre médias, e também são chamados
## de efeitos de Yates
dae::yates.effects(m0, error.term = "parcela")
## A metade da diferença entre médias são os coeficientes
## Enquanto os efeitos de Yates medem a diferença na variável resposta
## pela mudança em 2 unidades de cada fator, os coeficientes medem a
## diferença pela mudança em uma unidade em cada fator (igual aos
## modelos de regressão)
dae::yates.effects(m0, error.term = "parcela")/2
##----------------------------------------------------------------------
## Usando lm() com contraste soma zero
m0lm <- lm(var.resp ~ fatorA + fatorB + fatorA:fatorB,
           contrasts = list(fatorA = contr.sum,
                            fatorB = contr.sum))
## A ANOVA é a mesma
anova(m0lm)
## Os efeitos também estão aqui
summary(m0lm)
mean(var.resp) # média geral = intercepto
## Médias do fatorA
tapply(var.resp, fatorA, mean)
mean(var.resp) + coef(m0lm)[2] # media do fator A0
mean(var.resp) - coef(m0lm)[2] # media do fator A1
## Médias do fatorB
tapply(var.resp, fatorB, mean)
mean(var.resp) + coef(m0lm)[3] # media do fator B0
mean(var.resp) - coef(m0lm)[3] # media do fator B0
##----------------------------------------------------------------------
## E SE você considerasse a interação como não significativa?
m1 <- aov(var.resp ~ fatorA + fatorB + Error(parcela))
summary(m1)
dae::yates.effects(m1, error.term = "parcela")
## Usando lm()
m1lm <- lm(var.resp ~ fatorA + fatorB,
           contrasts = list(fatorA = contr.sum,
                            fatorB = contr.sum))
anova(m1lm)
summary(m1lm)

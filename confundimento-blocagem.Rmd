...
Comparando as três predições (sem blocos, com 2 blocos, e com 4 blocos),
notamos que ocorrem pequenas diferenças devido aos fatores presentes nos
modelos e ao confundimento entre blocos e interações.
...
como bloco. Os dados estão abaixo (disponíveis
[aqui](http://www.leg.ufpr.br/~fernandomayer/data/montgomery_14-37.txt)):
```{r, echo=FALSE}
...
```{r ex6, echo=FALSE, include=FALSE}
```
7. (Montgomery, DAE, ex. 6.26) Um experimento foi conduzido em uma
   fábrica de semicondutores com o objetivo de se aumentar o rendimento.
   Os fatores e níveis foram: A = abertura (pequena, grande), B =
   tempode exposição (20% abaixo do nominal, 20% acima do nominal), C =
   tempode desenvolvimento (30 s., 45 s.), D = dimensão da máscara
   (pequena, grande), E = tempo de gravação (14.5 min., 15.5 min.). Os
   resultados deste fatorial $2^5$ sem réplicas está disponível
   [aqui](http://leg.ufpr.br/~fernandomayer/data/montgomery_DAE_6-26.txt).
   Com isso:
   ...
   necessário.
```{r ex7, eval=FALSE, include=FALSE}
```
8. Usando os mesmos dados do exercício 7, construa e analise o
   experimento com 2 blocos, com ABCDE confundido com blocos.
```{r ex8, eval=FALSE, include=FALSE}
```
9. Repita o exercício 8, supondo que 4 blocos fossem necessários. Sugira
   um esquema de confundimento.
```{r ex9, eval=FALSE, include=FALSE}
```
10. Repita novamente o exercício 8, considerando 4 blocos com ACDE e BCD
(consequentemente ABE) confundidos com bloco.
```{r ex10, eval=FALSE, include=FALSE}
```

As respostas para os exercícios finais estão nesse
[script](scripts/script_confundimento-blocagem-respostas.R).

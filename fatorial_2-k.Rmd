...
Á medida que o número de fatores cresce em um experimento fatorial,
...
experimento que avalia a taxa de conversão de um processo, conforma a
...
xyplot(conversion ~ factor(x1) | x2 + x3, groups = x4,
       data = dados,  type = c("p", "l"), auto.key = TRUE)
...
gráfico de probabilidade normal para a estimativa dos efeitos. Lembre-se
que, sob a hipótses nula, todos os parâmetros do modelo são iguais a
zero, e pela definição do termo de erro do modelo, se os parâmetros são
iguais a zero, então sobra apenas o erro que possui média 0 e variância
constante $\sigma^2$. Dessa forma, se a hipótese nula for verdadeira,
esperamos que os efeitos tenham também média 0, e fiquem em cima da
linha em um gráfico de probabilidade normal. Os efeitos que se afastarem
muito da linha são aqueles que possuem média diferente de zero, e
portanto, são aqueles que temos interesse.
...
Note que a variabilidade das interações não consideradas foi integrada
na soma de quadrado dos resíduos, e assim podemos agora ter uma
estimativa para $\sigma^2$.

Com esse resultado, confirmamos a hipótese leventada no gráfico de
...
3. O fator pressão (`x3`) é essencialmente **inerte**, ou sem
   ...
   exemplo o custo ou o tempode operação.
4. Ver figura abaixo (mas acho que não vai ajudar).
...
xyplot(conversion ~ factor(x1) | x3, groups = interaction(x2,x4),
       data = dados,  type = c("p", "l"), auto.key = TRUE)
```
...
1. Fazer um novo experimento, agora com apenas estes dois fatores e com
um número maior de repetições: projeção
2. Fazer um novo experimento com estes dois fatores e, se desejado,
...
Para avaliar as suposições do modelo podemos fazer um gráfico de quantis
...
Como já vimos, as predições para experimentos fatorias nada mais são do
...
pred <- data.frame(x1 = dados$x1,
                   x2 = dados$x2,
                   x4 = dados$x4,
                   x2x4 = dados$x2 * dados$x4)
pred$y <- predict(m2, newdata = pred)
pred
## Verificar essas contas e combinar com proj()
proj(m2)
apply(proj(m2)[,-6], 1, sum)
cbind(pred, apply(proj(m2)[,-6], 1, sum))
## É isso mesmo!
```

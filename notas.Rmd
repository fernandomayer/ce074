...
## Nesse caso, existem duas notas P1P e P1D que precisam ser somadas. Se
## houver 1 NA, soma sem o NA. Se houverem 2 NAs, mantém NA.
notas$P1 <- apply(notas[, 2:3], 1, function(x){
    count.na <- sum(is.na(x))
    ifelse(count.na == 2, NA, sum(x, na.rm = TRUE))
})
...
- `P1P` é a nota da prova escrita feita em sala (valendo 9.0).
- `P1D` é a nota do Diagrama de Hasse entregue (valendo 1.0).
- `P1` é a nota final (`P1P` + `P1D`).

Observação: a nota `P1` será 70% do total, enquanto que a nota da prova
prática será 30% do total.

```{r, echo=FALSE, eval=TRUE, rownames.print=FALSE, rows.print=nrow(notas)}
notas[, c(1, c(2:3, notas.col))]
```
...
```{r, fig.show='hold', include=FALSE, eval=FALSE}
par(mfrow = c(1, 2))
barplot(prop.table(table(notas2$S))*100,
        xlab = "Situação", ylab = "Frequência (%)",
        main = "Antes do exame")
